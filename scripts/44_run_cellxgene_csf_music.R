...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[[1]]) else "scripts/44_run_cellxgene_csf_music.R"
...
write.table(
  weighted,
  file.path(table_dir, "npj_figure4_csf_music_proportions_wide.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  unweighted,
  file.path(table_dir, "npj_figure4_csf_music_proportions_allgene.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)

message("Wrote CELLxGENE CSF MuSiC proportions using ", length(common_genes), " common genes and ", ncol(sce), " reference cells.")
